...
Son test turunda unit testler, entegrasyon testi, Valgrind, ThreadSanitizer ve coverage ölçümü çalıştırıldı. Coverage `%85.48` seviyesine çıkarıldı ve Valgrind bellek sızıntısı raporlamadı. ThreadSanitizer WSL2 ortamında `setarch $(uname -m) -R make tsan` komutuyla çalıştırıldı; testler geçti ve data race raporu üretilmedi.
...
**Durum**: Ders projesi için test edildi; Valgrind, coverage ve TSan sonuçları günceldir.
