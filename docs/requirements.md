...
- `setarch $(uname -m) -R make tsan` ThreadSanitizer ile data race kontrolünü çalıştırmalıdır.
...
- ThreadSanitizer `setarch $(uname -m) -R make tsan` komutuyla başarılı çalışmıştır.
- TSan çalışmasında data race raporu üretilmemiştir.
